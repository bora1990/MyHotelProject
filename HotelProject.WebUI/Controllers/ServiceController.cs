...
namespace HotelProject.WebUI.Controllers
{
    public class ServiceController : Controller
    {
        ...
        public async Task<IActionResult> AddService(CreateServiceDto createServiceDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5251/api/Service", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5251/api/Service/{id}");  //api tarafında action üstünde [HttpDelete("{id}")] erişim içim için yazılmalıdır.
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5251/api/Service/{id}");
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceDto updateServiceDto)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(updateServiceDto);//jsona çevirdik
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "Application/Json");
            //API lere veri yollanırken Json tipi kullanılır.

            var responseMessage = await client.PutAsync("http://localhost:5251/api/Service/", stringContent);
